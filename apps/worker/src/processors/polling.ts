...
export async function processPolling(job: Job<PollJobData>) {
  const { generationJobId, providerTaskId, provider: providerName, attempt } = job.data;
  ...
  if (result.status === 'SUCCEEDED') {
    ...
    await publishJobStatus(generationJobId, 'RUNNING', { message: 'Downloading video...' });
    return;
  }
  ...
  if (result.status === 'FAILED' || result.status === 'CANCELED') {
    ...
    await publishJobStatus(generationJobId, 'FAILED', { error: result.errorMessage });
    return;
  }
  ...
  if (attempt >= MAX_POLL_ATTEMPTS) {
    await prisma.generationJob.update({
      where: { id: generationJobId },
      data: { status: 'FAILED', errorMessage: 'Polling timeout', finishedAt: new Date() },
    });
    await publishJobStatus(generationJobId, 'FAILED', { error: 'Polling timeout' });
    return;
  }

  const jitter = Math.random() * 2000;
  const delay = Math.min(5000 * Math.pow(1.5, Math.min(attempt, 10)), 60000) + jitter;
