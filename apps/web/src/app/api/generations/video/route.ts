...
import { prisma, Prisma } from '@ai-magic/db';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { outfitId, inputAssetId, provider, model, count, durationSec, resolution } = parsed.data;
    ...
    const outfit = await prisma.outfit.findUnique({
      where: { id: outfitId },
      include: { characterTemplate: true },
    });
    if (!outfit) {
      return NextResponse.json(fail('NOT_FOUND', '穿搭任务不存在'), {
        status: 404,
      });
    }

    const preset = outfit.scenarioPresetId
      ? await findOutfitScenario(outfit.scenarioPresetId)
      : null;
    const resolved = resolveVideoPromptFromOutfitRow(outfit, { preset });
    ...
    for (let i = 0; i < count; i++) {
      const genJob = await prisma.generationJob.create({
        data: {
          ...
          promptJson: resolved.promptJson as unknown as Prisma.InputJsonValue,
        },
      });
