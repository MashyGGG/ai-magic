...
export async function GET(req: NextRequest) {
  try {
    ...
    const jobCount = await prisma.generationJob.count({ where: where.createdAt ? { createdAt: where.createdAt as object } : {} });
    const successCount = await prisma.generationJob.count({
      where: { status: 'SUCCEEDED', ...(where.createdAt ? { createdAt: where.createdAt as object } : {}) },
    });
    const failedCount = await prisma.generationJob.count({
      where: { status: { in: ['FAILED', 'DOWNLOAD_FAILED'] }, ...(where.createdAt ? { createdAt: where.createdAt as object } : {}) },
    });
    ...
    return NextResponse.json(ok({
      ...
      successCount,
      failedCount,
      successRate: jobCount > 0 ? (successCount / jobCount * 100).toFixed(1) : '0',
    }));
  } catch (error) {
