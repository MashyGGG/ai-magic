...
import { CAMERA_TEMPLATES, MOTION_TEMPLATES, SCENE_TEMPLATES, buildImagePrompt } from '@ai-magic/prompts';
        ...
        <div className="lg:col-span-3">
          <Card>
            ...
            >
              ...
              <Form.Item name="characterTemplateId" label="角色模板" rules={[{ required: true, message: '请选择角色模板' }]}>
                <Select
                  placeholder="选择角色模板"
                  showSearch
                  optionFilterProp="label"
                  options={templates.map((t: { id: string; name: string }) => ({ value: t.id, label: t.name }))}
                />
              </Form.Item>

              <Collapse ghost defaultActiveKey={['outfit']} items={[
                {
                  key: 'outfit',
                  label: '穿搭描述',
                  children: (
                    <>
                      <Form.Item name="topDesc" label="上装"><Input placeholder="例：浅灰修身西装外套" /></Form.Item>
                      <Form.Item name="bottomDesc" label="下装"><Input placeholder="例：同色系短裙" /></Form.Item>
                      <div className="grid grid-cols-2 gap-x-4">
                        <Form.Item name="shoesDesc" label="鞋子"><Input placeholder="例：黑色尖头高跟鞋" /></Form.Item>
                        <Form.Item name="bagDesc" label="包"><Input placeholder="例：小号皮质手提包" /></Form.Item>
                      </div>
                      <Form.Item name="accessoriesDesc" label="配饰"><Input placeholder="例：金属耳环" /></Form.Item>
                      <div className="grid grid-cols-2 gap-x-4">
                        <Form.Item name="materialDesc" label="面料/材质"><Input placeholder="例：西装面料有轻微纹理" /></Form.Item>
                        <Form.Item name="colorDesc" label="配色"><Input placeholder="例：浅灰 + 黑色点缀" /></Form.Item>
                      </div>
                    </>
                  ),
                },
                {
                  ...
                  children: (
                    <>
                      <Form.Item name="backgroundDesc" label="背景描述"><Input placeholder="例：干净高级的都市摄影棚" /></Form.Item>
                      <div className="grid grid-cols-2 gap-x-4">
                        <Form.Item name="cameraTemplate" label="镜头模板">
                          <Select allowClear placeholder="选择" options={CAMERA_TEMPLATES.map((c) => ({ value: c.id, label: c.label }))} />
                        </Form.Item>
                        <Form.Item name="motionTemplate" label="动作模板">
                          <Select allowClear placeholder="选择" options={MOTION_TEMPLATES.map((m) => ({ value: m.id, label: m.label }))} />
                        </Form.Item>
                      </div>
                      <Form.Item name="sceneDesc" label="场景模板">
                        <Select allowClear placeholder="选择" options={SCENE_TEMPLATES.map((s) => ({ value: s.id, label: s.label }))} />
                      </Form.Item>
                    </>
                  ),
                },
              ...
              <div className="mt-6">
                <Button type="primary" htmlType="submit" size="large" loading={createMutation.isPending} block>
                  创建任务
                </Button>
              </div>
            </Form>
          ...
          <div className="sticky top-4 space-y-4">
            <Card title="预计成本" size="small">
              <div className="grid grid-cols-2 gap-4">
                <Statistic title="图片成本" value={costEstimate.imgCost} precision={2} prefix="¥" valueStyle={{ color: '#c9a96e' }} />
                <Statistic title="视频成本" value={costEstimate.vidCost} precision={2} prefix="¥" valueStyle={{ color: '#c9a96e' }} />
              </div>
              <div className="mt-3 border-t pt-3">
                <Statistic title="预计总计" value={costEstimate.total} precision={2} prefix="¥" valueStyle={{ color: '#c9a96e', fontSize: 24 }} />
                <Text type="secondary" className="text-xs">含重试预算约 ¥{(costEstimate.total * 2).toFixed(2)}</Text>
              </div>
            </Card>
            ...
            <Card title="Prompt 预览" size="small">
              {promptPreview ? (
                ...
                <Paragraph type="secondary">选择角色模板并填写穿搭信息后，将自动生成 Prompt 预览</Paragraph>
              )}
            </Card>
