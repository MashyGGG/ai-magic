...
import { CAMERA_TEMPLATES, MOTION_TEMPLATES, SCENE_TEMPLATES } from '@ai-magic/prompts';
...
export function TemplateFormDrawer({ open, editId, onClose, onSuccess }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: Record<string, unknown>) => {
      const url = editId ? `/api/character-templates/${editId}` : '/api/character-templates';
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-x-4">
          <Form.Item name="defaultCamera" label="默认镜头">
            <Select placeholder="选择" allowClear options={CAMERA_TEMPLATES.map((c) => ({ value: c.id, label: c.label }))} />
          </Form.Item>
          <Form.Item name="defaultMotion" label="默认动作">
            <Select placeholder="选择" allowClear options={MOTION_TEMPLATES.map((m) => ({ value: m.id, label: m.label }))} />
          </Form.Item>
        </div>

        <Form.Item name="defaultScene" label="默认场景">
          <Select placeholder="选择" allowClear options={SCENE_TEMPLATES.map((s) => ({ value: s.id, label: s.label }))} />
        </Form.Item>
        ...
        <Form.Item label="参考图">
          ...
          <Form.Item name="referenceAssetId" hidden><Input /></Form.Item>
        </Form.Item>
      </Form>
    </Drawer>
  );
